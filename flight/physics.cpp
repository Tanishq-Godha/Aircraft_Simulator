...
void updateThrottle(float dt) {
    ...
    if (controllerConnected) {
        float rT = controllerState.Gamepad.bRightTrigger / 255.0f;
        float lT = controllerState.Gamepad.bLeftTrigger / 255.0f;
        if (rT > 0.1f) throttle += rT * 1.8f * dt; // RT increases throttle
        if (lT > 0.1f) throttle -= lT * 1.8f * dt; // LT decreases throttle
    }
#endif
...
void updateLandingGear(float dt) {
    ...
    // Always move gear door if a target change is commanded or gear is not at target
    bool needsToMove = std::fabs(gearAnimation - gearTarget) > 0.01f;
    
    if (needsToMove) {
        // Open door completely
        gearDoorAnim = approach(gearDoorAnim, 1.0f, 8.0f, dt);
        // Only move gear if door is sufficiently open
        if (gearDoorAnim > 0.6f) {
            gearAnimation = approach(gearAnimation, gearTarget, 4.0f, dt);
        }
    } else {
        // Ensure animation is pegged
        gearAnimation = gearTarget;
        // Close door
        gearDoorAnim = approach(gearDoorAnim, 0.0f, 8.0f, dt);
    }
    
    gearInTransition = needsToMove || gearDoorAnim > 0.02f;
}
...
#ifdef _WIN32
    if (controllerConnected) {
        float stickLX = controllerState.Gamepad.sThumbLX / 32767.0f;
        if (std::fabs(stickLX) > 0.2f) { // Deadzone
            yaw -= stickLX * steerRate * dt * 0.7f;
        }

        if (controllerState.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) yaw -= steerRate * dt * 0.7f;
        if (controllerState.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER)  yaw += steerRate * dt * 0.7f;
    }
#endif
...
#ifdef _WIN32
    if (controllerConnected) {
        float stickRY = controllerState.Gamepad.sThumbRY / 32767.0f;
        if (std::fabs(stickRY) > 0.2f) {
            pitchTarget -= stickRY * 10.0f;
        }
    }
#endif
...
#ifdef _WIN32
        if (controllerConnected && !autopilotOn) {
            float stickLX = controllerState.Gamepad.sThumbLX / 32767.0f;
            if (std::fabs(stickLX) > 0.2f) {
                targetRoll += stickLX * 45.0f; // Left Stick X -> Roll (Right/Left)
            }
            
            float stickRY = controllerState.Gamepad.sThumbRY / 32767.0f;
            if (std::fabs(stickRY) > 0.2f) {
                pitchInput -= stickRY * 30.0f;  // Right Stick Y -> Pitch (Down = Pitch Up, Up = Pitch Down)
            }
            
            if (controllerState.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) yaw -= 30.0f * dt;
            if (controllerState.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER)  yaw += 30.0f * dt;
        }
#endif

        pitch += pitchInput * dt;

        if (std::fabs(pitchInput) < 0.1f) {
...
void simulatePhysics(float dt) {
    ...
    // Update landing gear independent of grounded state to allow ground retraction belly-flops
    ...
    if (!isExploding && !crashed && !isGrounded && planeY <= newGround + clearance) {
        ...
        bool hardTouchdown = isHardLanding(sinkRate) || (isSafeLanding(sinkRate) && !onValidRunway); // Off-runway becomes a hard landing
        ...
        if (safeTouchdown || hardTouchdown) {
            ...
            cameraTrauma += hardTouchdown ? 0.4f : 0.1f; // Camera shake on landing
        } else {
            isExploding = true;
            explosionTimer = 0.0f;
            planeY  = newGround + clearance;
            currentSpeed *= 0.5f;
            cameraTrauma += 1.0f; // Maximum camera shake
        }
    }
