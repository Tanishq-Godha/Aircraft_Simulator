...
float getSceneHeight(float x, float z) {
    ...
    // --- 0. TWIN TOWERS OVERRIDE ---
    if (spawnTwinTowers) {
        float tw = 360.0f;
        float td = 360.0f;
        float th = 2400.0f * twinTowerAnim;
        float spacing = 250.0f;

        // Tower 1 (Left)
        float t1X = twinTowerX - spacing;
        if (x >= t1X - tw/2.0f && x <= t1X + tw/2.0f &&
            z >= twinTowerZ - td/2.0f && z <= twinTowerZ + td/2.0f) {
            if (th > maxObjH) maxObjH = th;
        }
        
        // Tower 2 (Right)
        float t2X = twinTowerX + spacing;
        if (x >= t2X - tw/2.0f && x <= t2X + tw/2.0f &&
            z >= twinTowerZ - td/2.0f && z <= twinTowerZ + td/2.0f) {
            if (th > maxObjH) maxObjH = th;
        }
    }
...
void drawTwinTowers() {
    if (!spawnTwinTowers) return;

    // Animate
    if (twinTowerState == 1) {
        twinTowerAnim += deltaTime * 0.1f; // Take ~10 seconds to fully rise
        if (twinTowerAnim >= 1.0f) {
            twinTowerAnim = 1.0f;
            twinTowerState = 2; // Finished rising
        }
    }
    ...
    float groundY = getVoxelHeight(twinTowerX, twinTowerZ);

    // Realistic steel/silver grey and slightly darker vertical core (if possible)
    float r = 0.70f, g = 0.72f, b = 0.75f;
    float winR = 0.2f, winG = 0.22f, winB = 0.25f;

    // Tower 1 (Left / North)
    float t1x = twinTowerX - spacing;
    // Core (dark windows)
    drawCube(t1x, groundY + th / 2.0f, twinTowerZ, tw, th, td, winR, winG, winB);
    
    // Draw 4 corner columns to give the iconic chamfered/ribbed look
    float cTw = tw * 0.2f; // Column thickness
    // Front Left, Front Right, Back Left, Back Right
    drawCube(t1x - tw/2 + cTw/2, groundY + th/2, twinTowerZ - td/2 + cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t1x + tw/2 - cTw/2, groundY + th/2, twinTowerZ - td/2 + cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t1x - tw/2 + cTw/2, groundY + th/2, twinTowerZ + td/2 - cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t1x + tw/2 - cTw/2, groundY + th/2, twinTowerZ + td/2 - cTw/2, cTw, th, cTw, r, g, b);
    
    // Plus a middle vertical column on each face for the classic pinstripe look
    drawCube(t1x, groundY + th/2, twinTowerZ - td/2 + cTw/8, tw, th, cTw/4, r, g, b); // Front face
    drawCube(t1x, groundY + th/2, twinTowerZ + td/2 - cTw/8, tw, th, cTw/4, r, g, b); // Back face
    drawCube(t1x - tw/2 + cTw/8, groundY + th/2, twinTowerZ, cTw/4, th, td, r, g, b); // Left face
    drawCube(t1x + tw/2 - cTw/8, groundY + th/2, twinTowerZ, cTw/4, th, td, r, g, b); // Right face
    
    // Iconic North Tower Antenna
    float antH = 530.0f * twinTowerAnim;
    drawCube(t1x, groundY + th + antH / 2.0f, twinTowerZ, 25.0f, antH, 25.0f, 0.5f, 0.5f, 0.5f);
    
    // Tower 2 (Right / South)
    float t2x = twinTowerX + spacing;
    // Core (dark windows)
    drawCube(t2x, groundY + th / 2.0f, twinTowerZ, tw, th, td, winR, winG, winB);
    
    // 4 corner columns
    drawCube(t2x - tw/2 + cTw/2, groundY + th/2, twinTowerZ - td/2 + cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t2x + tw/2 - cTw/2, groundY + th/2, twinTowerZ - td/2 + cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t2x - tw/2 + cTw/2, groundY + th/2, twinTowerZ + td/2 - cTw/2, cTw, th, cTw, r, g, b);
    drawCube(t2x + tw/2 - cTw/2, groundY + th/2, twinTowerZ + td/2 - cTw/2, cTw, th, cTw, r, g, b);
    
    // Middle vertical ribs
    drawCube(t2x, groundY + th/2, twinTowerZ - td/2 + cTw/8, tw, th, cTw/4, r, g, b);
    drawCube(t2x, groundY + th/2, twinTowerZ + td/2 - cTw/8, tw, th, cTw/4, r, g, b);
    drawCube(t2x - tw/2 + cTw/8, groundY + th/2, twinTowerZ, cTw/4, th, td, r, g, b);
    drawCube(t2x + tw/2 - cTw/8, groundY + th/2, twinTowerZ, cTw/4, th, td, r, g, b);
}
